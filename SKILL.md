...
## 운영 모드 — Claude 주도, 사용자는 검수자 (최우선)

blueprint가 시작되면 **사용자가 무엇을 말해야 할지 몰라도 진행되게** 한다. "이제 뭐 하죠?"라고 사용자가 묻게 만들면 실패. 매 phase에서 Claude가:

1. 그 phase가 요구하는 항목을 **먼저 펼쳐 보여준다** (체크리스트 = `plans/roadmap.md`).
2. INQUIRY·PRODUCT·웹 리서치·이전 답변에서 **답안 초안을 직접 작성**한다 — 빈칸을 사용자에게 떠넘기지 않는다.
3. 갈림길·미정·취향이 필요한 것만 **하나씩 질문**한다 (절대 원칙 #1·#2: 한 번에 하나, 추천 먼저).
4. phase 끝에 결과를 **검수받는다** ("이대로 `docs/{file}.md`에 저장할까요?").
5. 사용자 답에서 빈 곳·모순이 드러나면 **Claude가 먼저 더 캐물어** 채운다 (사용자가 "더 자세히"라고 안 해도).
6. **미결 게이트**(Step 1.5 / 전환 점검) 통과 후에만 다음 phase로.

즉 **사용자는 결정·검수만, 무엇을 물을지는 Claude가 안다.** 모든 phase(INQUIRY~POST-SHIP)에 이 모드가 적용된다.
...
**폰트는 반드시 명시적으로 질문한다** (디자인 시스템 단계의 필수 사항):
- "제품 폰트를 무엇으로 할까요?"를 AskUserQuestion으로 묻되, **한글+영문 페어링 후보 3~4개를 추천(첫 옵션)으로 제시**한다. 예:
  - Pretendard(한글) + Inter(영문) — 깔끔·범용 (기본 추천)
  - Pretendard + 시스템 산세리프 — 가벼움
  - Noto Sans KR + Roboto — 안드로이드/구글 톤
  - 명조 계열(Nanum Myeongjo 등) — 진중·에디토리얼
- 고른 폰트를 `docs/DESIGN.md`의 `## Typography` (Heading/Body family)에 박는다 → 대시보드 Preview 탭이 자동으로 폰트 토큰 샘플로 표시.
